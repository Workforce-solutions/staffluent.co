        ...
        if (
          ...
          (response.status === "already_verified" &&
            (response.subscriptionStatus === "incomplete" ||
              response.subscriptionStatus === "trialing"))
        ) {
          setVerificationStatus("success");
          redirectTimer = setTimeout(() => {
            router.push(
              `/subscription?userId=${response.userId}&businessId=${response.businessId}`
            );
          }, 3000);
        }
              ...
              <div className="max-w-md w-full">
                ...
                {verificationStatus === "error" && (
                  <div className="bg-white p-8 rounded-2xl border border-[#DEE5ED] shadow-sm text-center">
                    ...
                    <p className="text-[#3D495B] mb-6">
                      We couldn&apos;t verify your email. The link might be
                      invalid.
                    </p>
                    <button
                      onClick={() => router.push("/contact")}
